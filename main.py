    ...
    if PokemonValue != "":
        try:
            ...
            if not user_row_index.empty:
                ...
                print(f"Pokemon {PokemonValue} added successfully for {UserName}!")
            else:
...
if success:
    ...
    with open("users.csv", "r") as csv:
        df = pd.read_csv(csv, usecols=['username', 'pokemon1', 'pokemon2', 'pokemon3', 'pokemon4', 'pokemon5', 'pokemon6'])
        df = df.loc[df['username'] == UserName]
        print(UserName)
        print(df)
        poke1 = tk.Label(text=df['pokemon1'].values[0] if pd.notna(df['pokemon1'].values[0]) else "None")
        poke2 = tk.Label(text=df['pokemon2'].values[0] if pd.notna(df['pokemon2'].values[0]) else "None")
        poke3 = tk.Label(text=df['pokemon3'].values[0] if pd.notna(df['pokemon3'].values[0]) else "None")
        poke4 = tk.Label(text=df['pokemon4'].values[0] if pd.notna(df['pokemon4'].values[0]) else "None")
        poke5 = tk.Label(text=df['pokemon5'].values[0] if pd.notna(df['pokemon5'].values[0]) else "None")
        poke6 = tk.Label(text=df['pokemon6'].values[0] if pd.notna(df['pokemon6'].values[0]) else "None")
...
# If registering, create the registration page
    ...
    def register_user():
        try:
            ...
            CSV = pd.DataFrame(columns=['username', 'password', 'pokemon1', 'pokemon2', 'pokemon3', 'pokemon4', 'pokemon5', 'pokemon6'])
        ...
        if user == "" or password == "":
            ...
            if CSV['username'].isin([user]).any():
                ...
                new_data = {'username': user, 'password': password, 'pokemon1': "", 'pokemon2': "", 'pokemon3': "", 'pokemon4': "", 'pokemon5': "", 'pokemon6': ""}
